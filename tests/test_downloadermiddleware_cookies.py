...
class CookiesMiddlewareTest(TestCase):
    ...
    def test_basic(self):
        ...
        self.assertIs(self.mw.process_response(req, res, self.spider), res)

        req2 = Request('http://scrapytest.org/sub1/')
        self.assertIsNone(self.mw.process_request(req2, self.spider))
    ...
    def test_complex_cookies(self):
        ...
        self.mw.process_request(req, self.spider)
        self.assertIn(req.headers.get('Cookie'), (b'C1=value1; C3=value3', b'C3=value3; C1=value1'))
    ...
    def test_cookiejar_key(self):
        ...
        self.assertIsNone(self.mw.process_request(req3, self.spider))
        ...
        self.assertIs(self.mw.process_response(req3, res2, self.spider), res2)

        req4 = Request('http://scrapytest.org/', meta=res2.meta)
        self.assertIsNone(self.mw.process_request(req4, self.spider))
        ...
        self.assertIsNone(self.mw.process_request(req5_1, self.spider))

        headers = {'Set-Cookie': 'C1=value1; path=/'}
        res5_1 = Response('http://scrapytest.org:1104/', headers=headers, request=req5_1)
        self.assertIs(self.mw.process_response(req5_1, res5_1, self.spider), res5_1)

        req5_2 = Request('http://scrapytest.org:1104/some-redirected-path')
        self.assertIsNone(self.mw.process_request(req5_2, self.spider))
        self.assertEqual(req5_2.headers.get('Cookie'), b'C1=value1')

        req5_3 = Request('http://scrapytest.org/some-redirected-path')
        self.assertIsNone(self.mw.process_request(req5_3, self.spider))
        ...
        self.assertIsNone(self.mw.process_request(req6, self.spider))
        self.assertEqual(req6.headers.get('Cookie'), None)

    def test_local_domain(self):
        request = Request("http://example-host/", cookies={'currencyCookie': 'USD'})
        self.assertIsNone(self.mw.process_request(request, self.spider))
    ...
    def test_keep_cookie_from_default_request_headers_middleware(self):
        ...
        self.assertIsNone(mw_default_headers.process_request(req1, self.spider))
        self.assertIsNone(self.mw.process_request(req1, self.spider))
        self.assertCookieValEqual(req1.headers['Cookie'], b'default=something; asdf=qwerty')
        # keep both
        req2 = Request('http://example.com', cookies={'a': 'b'})
        self.assertIsNone(mw_default_headers.process_request(req2, self.spider))
        self.assertIsNone(self.mw.process_request(req2, self.spider))
