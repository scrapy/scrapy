...
def get_engine_class() -> type[ExecutionEngine]:
    crawler = get_crawler(MySpider)
    engine_class = load_object(crawler.settings.get("EXECUTION_ENGINE_CLASS"))
    if not issubclass(engine_class, ExecutionEngine):
        raise TypeError(
            f"The provided engine class ({crawler.settings['EXECUTION_ENGINE_CLASS']})"
...
class TestEngine(TestEngineBase):
    ...
    async def test_close_downloader(self):
        e = get_engine_class()(get_crawler(MySpider), lambda _: None)
        await e.close_async()
    ...
    def test_start_already_running_exception(self):
        ...
        e = get_engine_class()(crawler, lambda _: None)
...
class TestEngineDownloadAsync:
    ...
    def engine(self) -> ExecutionEngine:
        crawler = get_crawler(MySpider)
        engine = get_engine_class()(crawler, lambda _: None)
...
class TestEngineCloseSpider:
    ...
    async def test_exception_callback(
        ...
        engine = get_engine_class()(crawler, lambda _: defer.fail(ValueError()))
        await engine.open_spider_async()
        await engine.close_spider_async()
    ...
    async def test_exception_async_callback(
        ...
        engine = get_engine_class()(crawler, cb)
        await engine.open_spider_async()
        await engine.close_spider_async()
        assert "Error running spider_closed_callback" in caplog.text
