...
class SimpleSpider(MetaSpider):
    ...
    def parse(self, response):
        self.logger.info(f"Got response {str(response.status)}")
...
class AsyncDefAsyncioSpider(SimpleSpider):
    ...
    async def parse(self, response):
        ...
        self.logger.info(f"Got response {str(status)}")
...
class AsyncDefAsyncioReqsReturnSpider(SimpleSpider):
    ...
    async def parse(self, response):
        ...
        status = await get_from_asyncio_queue(response.status)
        self.logger.info(f"Got response {str(status)}, {str(req_id)}" % (status, req_id))
        if req_id > 0:
            return
        reqs = []
        for i in range(1, 3):
