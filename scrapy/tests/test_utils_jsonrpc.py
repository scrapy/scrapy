...
class JsonRpcUtilsTestCase(unittest.TestCase):
    ...
    def test_jsonrpc_client_call_args_kwargs_raises(self):
        self.assertRaises(
            ValueError, jsonrpc_client_call, 'url', 'test', 'one', kw=123)
    ...
    def test_jsonrpc_client_call_response(self):
        ...
        self.assertRaises(
            ValueError, jsonrpc_client_call, 'url', 'test', _urllib=ul)
        ul = urllib_mock(result={'one': 1})
        self.assertEquals(
            jsonrpc_client_call('url', 'test', _urllib=ul), {'one': 1})
        ul = urllib_mock(
            error={'code': 123, 'message': 'hello', 'data': 'some data'})
    ...
    def test_jsonrpc_server_call(self):
        ...
        r = jsonrpc_server_call(
            t, '{"method": "notfound", "id": 1}', self.json_decoder)
        assert 'error' in r
        assert r['jsonrpc'] == '2.0'
        assert r['id'] == 1
        ...
        r = jsonrpc_server_call(
            t, '{"method": "exception", "id": 1}', self.json_decoder)
        assert 'error' in r
        assert r['jsonrpc'] == '2.0'
        assert r['id'] == 1
        ...
        r = jsonrpc_server_call(
            t, '{"method": "call", "id": 2}', self.json_decoder)
        assert 'result' in r
        assert r['jsonrpc'] == '2.0'
        assert r['id'] == 2
