...
class RedirectMiddlewareTest(unittest.TestCase):
    ...
    def test_redirect_301(self):
        def _test(method):
            ...
            # response without Location header but with status code is 3XX
            # should be ignored
    ...
    def test_redirect_302(self):
        ...
        # response without Location header but with status code is 3XX should
        # be ignored
        del rsp.headers['Location']
        assert self.mw.process_response(req, rsp, self.spider) is rsp
    ...
    def test_ttl(self):
        ...
        rsp = Response('http://www.scrapytest.org/302',
                       headers={'Location': '/redirected'}, status=302)
    ...
    def test_redirect_urls(self):
        ...
        rsp1 = Response(
            'http://scrapytest.org/first', headers={'Location': '/redirected'}, status=302)
        req2 = self.mw.process_response(req1, rsp1, self.spider)
        rsp2 = Response('http://scrapytest.org/redirected',
                        headers={'Location': '/redirected2'}, status=302)
...
class MetaRefreshMiddlewareTest(unittest.TestCase):
    ...
    def test_meta_refresh_with_high_interval(self):
        ...
        req = Request(url='http://example.org')
        rsp = HtmlResponse(
            url='http://example.org', body=self._body(interval=1000))
