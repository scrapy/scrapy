...
class CsvItemExporter(BaseItemExporter):
    ...
    def export_item(self, item: Any) -> None:
        ...
        if self._autodetected_fields and self.fields_to_export is not None:
            item_fields = ItemAdapter(item).field_names()
            dropped_fields = set(item_fields) - set(self.fields_to_export)

            if dropped_fields and not self._data_loss_warned:
                logger.warning(
                    f"CSVExporter dropped fields {dropped_fields}. "
