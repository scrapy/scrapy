...
class Command(ScrapyCommand):
    ...
    def process_options(self, args, opts):
        ...
        if opts.output:
            ...
            valid_output_formats = self.settings[
                'FEED_EXPORTERS'].keys() + self.settings['FEED_EXPORTERS_BASE'].keys()
            if opts.output_format not in valid_output_formats:
                raise UsageError('Invalid/unrecognized output format: %s, Expected %s' %
