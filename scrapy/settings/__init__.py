...
class BaseSettings(MutableMapping[_SettingsKeyT, Any]):
    ...
    def getdictorlist(
        ...
        if isinstance(value, str):
            try:
                ...
                if not isinstance(value_loaded, (dict, list)):
                    raise ValueError(f"The value for setting must be a dict or list, got {type(value_loaded).__name__}")
                return value_loaded
            except ValueError:
                return value.split(",")
        ...
        if not isinstance(value, (dict, list)):
            raise ValueError(f"The value for setting must be a dict or list, got {type(value).__name__}")
