...
class HttpCacheMiddleware(object):
    ...
    @classmethod
    def from_crawler(cls, crawler: Type[Crawler]) -> HttpCacheMiddleware:
        o: HttpCacheMiddleware = cls(crawler.settings, crawler.stats)
        crawler.signals.connect(o.spider_opened, signal=signals.spider_opened)
        crawler.signals.connect(o.spider_closed, signal=signals.spider_closed)
        return o

    def spider_opened(self, spider: Type[Spider]) -> None:
        self.storage.open_spider(spider)

    def spider_closed(self, spider: Type[Spider]) -> None:
        self.storage.close_spider(spider)

    def process_request(self, request: Type[Request], spider: Type[Spider]) -> Optional[Type[Response]]:
        ...
        cachedresponse: Type[Response] = self.storage.retrieve_response(spider, request)
    ...
    def process_response(self, request: Type[Request], response: Type[Response], spider: Type[Spider]) -> Type[Response]:
        ...
        cachedresponse: Optional[Type[Response]] = request.meta.pop('cached_response', None)
        if cachedresponse is None:
    ...
    def process_exception(self, request: Type[Request], exception: Type[Exception], spider: Type[Spider]) -> Optional[Type[Response]]:
        cachedresponse: Type[Response] = request.meta.pop('cached_response', None)
        if cachedresponse is not None and isinstance(exception, self.DOWNLOAD_EXCEPTIONS):
            self.stats.inc_value('httpcache/errorrecovery', spider=spider)
            return cachedresponse

    def _cache_response(self, spider: Type[Spider], response: Type[Response], request: Type[Request], cachedresponse: Optional[Type[Response]]) -> None:
