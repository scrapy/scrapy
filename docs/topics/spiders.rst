...
.. autoclass:: Rule
    ...
    .. attribute:: Rule.link_extractor
        A :ref:`Link Extractor <topics-link-extractors>` object which
    ...
    .. attribute:: Rule.callback
        A callable or a string (in which case a method from the spider
    ...
    .. attribute:: Rule.cb_kwargs
        A dict containing the keyword arguments to be passed to the
        callback function.
    .. attribute:: Rule.follow
        A boolean which specifies if links should be followed from each
        response extracted with this rule. If ``callback`` is None ``follow`` defaults
        to ``True``, otherwise it defaults to ``False``.
    .. attribute:: Rule.process_links
        A callable, or a string (in which case a method from the
    ...
    .. attribute:: Rule.process_links
        A callable (or a string, in which case a method from
