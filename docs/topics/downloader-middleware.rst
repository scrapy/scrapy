...
.. _httpcache-storage-warc:

Web ARChive storage backend (WARC)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. class:: WARCCacheStorage

    Web ARChive storage backend is available for the HTTP cache middleware.

    This storage backend simplifies replaying a crawl, as well as to store
    the crawl data in a format that can be used by other tools.  All relevant
    cache data for a given URL is stored in a single file -in contrast to
    FilesystemCacheStorage, which spreads the data across multiple files and
    folders.

    Each request-response pair is stored as a pair of WARC records in a
    .warc file, in the order- Response, Request -for convenience.  (This order
    was chosen for convenience, and the order does not affect cache replay.)

    The WARC file name generated by this backend is based on the path style
    chosen from enum HTTPCacheWARCPathStyle, with a .gz suffix added if gzip
    is enabled.  The 'FOLDERS' name styles allow pasting the URL (without
    scheme), and with tab completion getting the complete filename.  Some
    filesystems many not support all URL characters, so the 'FILE_ONLY' and
    'FILE_AND_HASH' styles should be used in those cases.

    For examples, for the url- https://scrapeme.live/shop/Ivysaur/

    - FILE_ONLY:
        /cache_dir/scrapeme.live/shop_Ivysaur.warc[.gz]
    - FILE_AND_HASH:
        /cache_dir/scrapeme.live/shop_Ivysaur-0c961e2ea80fa4401a8d429c51ecf67298697be5.warc[.gz]
    - FOLDERS_AND_FILE:
        /cache_dir/scrapeme.live/shop/Ivysaur.warc[.gz]
    - FOLDERS_FILE_AND_HASH:
        /cache_dir/scrapeme.live/shop/Ivysaur/0c961e2ea80fa4401a8d429c51ecf67298697be5.warc[.gz]

    GNU/Linux command-line tools, such as `metawarc` and `less` can be used to
    review and test the cached WARC files.
